...
public interface LoanRepository extends JpaRepository<Loan, Long> {
    ...
    @Query("SELECT COUNT(l) FROM Loan l WHERE l.account.profile.idProfile = :profileId")
    ...
    @Query("SELECT COALESCE(SUM(l.ammountRequest), 0) FROM Loan l WHERE l.loanStatus = 'CLOSED' AND l.account.profile.idProfile = :profileId")
