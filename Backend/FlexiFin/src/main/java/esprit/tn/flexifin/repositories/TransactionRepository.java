...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    //Excel
    @Query("SELECT t FROM Transaction t WHERE (:accountIds IS NULL OR t.account" +
            ".idAccount IN :accountIds) AND (:startDate IS NULL OR t.date >= :startDate) AND (:endDate IS NULL OR t.date <= :endDate)")
