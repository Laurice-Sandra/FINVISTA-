...
@Override
    ...
    public String createStripeSession(Long accountId) throws StripeException {
        String successUrl = "http://localhost:8080/api/transactions/payment-success?session_id={CHECKOUT_SESSION_ID}&account_id=" + accountId;
        String cancelUrl = "http://localhost:8080/api/transactions/payment-cancelled";
    ...
    public ByteArrayInputStream generateFlexibleReportExcel(List<Long> accountIds, List<Integer> years, List<Integer> months) throws IOException {
        ...
        if (!years.isEmpty() && !months.isEmpty()) {
            startDate = LocalDate.MAX.of(Collections.min(years), Collections.min(months), 1);
