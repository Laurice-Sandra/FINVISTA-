  ...
  implements OnInit
{
  ...
  exampleDatabase?: TicketsService;
  dataSource!: ExampleDataSource;
  selection = new SelectionModel<Tickets>(true, []);
  index?: number;
  id?: number;
  ticket?: Tickets;
  constructor(
    ...
    this.subs.sink = dialogRef.afterClosed().subscribe((result) => {
      if (result === 1) {
        ...
        this.exampleDatabase?.dataChange.value.unshift(
          this.ticketsService.getDialogData()
        );
        this.refreshTable();
  ...
  deleteItem(i: number, row: Tickets) {
    this.index = i;
    this.id = row.id;
    let tempDirection: Direction;
    if (localStorage.getItem('isRtl') === 'true') {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.refreshTable();
      this.selection = new SelectionModel<Tickets>(true, []);
    });
    this.showNotification(
      'snackbar-danger',
...
export class ExampleDataSource extends DataSource<Tickets> {
  ...
  renderedData: Tickets[] = [];
  constructor(
    public exampleDatabase: TicketsService,
